...
public class InsertNewServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet InsertNewServlet</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet InsertNewServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if (images == null || title == null || des == null || date == null) {
                ...
                response.sendRedirect("InsertNewServlet");
            } else {
                String imagesNew = "images/" + images;
                NewsDAO news = new NewsDAO();
                news.insertNews(title, content, des, date, imagesNew);
                response.sendRedirect("InsertNewServlet");
            }
        } catch (Exception e) {
            request.setAttribute("Erroe", "Input Full Field");
